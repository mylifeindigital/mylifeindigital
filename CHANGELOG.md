...
- Gave `CR-030` its detail file and settled all six of its open questions, then shipped phase 1. Reconnaissance found half the request overtaken: it was specified to surface pipeline warnings "collected into `context.warnings` and discarded", but `CR-028` has since made an unprocessable file fatal and `CR-013` routes validation issues to pull-request annotations before merge — measured against the real tree, a console built to that spec renders an empty table today (81 items, **0 issues, 0 failures**). What survives is constrained by `wrangler.toml`'s standing rule that the Worker holds no secrets, so the only source is a build-time stamp: not a report *about* a deployment but the deployment describing itself, and therefore unable to be stale — at the cost of being structurally unable to describe a deployment that **failed**, since a failed deploy ships no Worker and the previous one keeps answering with its own commits, correctly and indistinguishably from a broken page. That is scoped out, with the R2 record that would have carried it. The deciding question — Worker route or CLI — turned out to be the mirror image of `CR-018`'s authoring rejection rather than a repeat of it: a checkout beats a browser for writing a file, and loses entirely for describing production, because `posts-data.ts` is git-ignored and rebuilt per deploy, so the artifact defining what the site contains exists in no checkout at all.

- Added `CR-035`: `generate:images` has no draft awareness and generated images for four unpublished posts on its first real run. `discoverContent` parses each file's frontmatter and records exactly one thing about it — `hasCustomImage` — and never reads `data.draft`. The build pipeline has filtered drafts since long before this, and `CR-013` made the ordering a tested guarantee on the reasoning that a draft is unfinished by definition; the generator sits outside the pipeline and inherited none of it. Caught by arithmetic rather than by reading the code: building the merged content produced 16 distinct image URLs where 20 items carried image frontmatter, because `DraftFilterProcessor` drops the four drafts. Premature spend rather than pure waste — the images are ready if those drafts publish — except that the manifest keys on a hash of the body, so a draft rewritten before publication regenerates anyway, and an abandoned one leaves an orphaned manifest entry and two orphaned R2 objects.
...
- Completed `CR-030` across three phases (`0.11.0`–`0.13.0`). The site answers what is live at `/status`, and the Worker still holds no credentials — `wrangler.toml`'s "this Worker has no secrets" survives the request unamended, which was the constraint that shaped everything else. The question turned out to be narrower than `CR-018` imagined and sharper than it looked: the site compiles its content into the Worker, so "what does production contain" is answerable from no repository, and `posts-data.ts` is git-ignored and rebuilt per deploy, so the artifact defining the site exists in no checkout at all. Only the running Worker knows, and now it says so.

  What the request did not build is as much of the outcome as what it did. Two of `CR-018`'s four reports had been overtaken before implementation started, and the third — the last deployment's outcome — was scoped out because a build-time stamp structurally cannot carry it: a failed deploy ships no Worker, so the previous one keeps serving and keeps reporting its own commits, correctly and indistinguishably from a page that has stopped updating. The page says what it describes, and `.github/DEPLOYMENT.md` records which surface answers "what is live" and which answers "what happened". What shipped is one honest question and a small surface answering it, rather than the dashboard the name suggests.

  Three findings worth keeping. The route-versus-CLI decision is the **mirror image** of `CR-018`'s authoring rejection rather than a repeat of it: a checkout beats a browser for writing a file and loses entirely for describing production, because it is not production. **Stamp only what the artifact does not already carry** — the phase plan called for stamping per-section counts, but `posts-data.ts` ships in the same bundle and already holds the items, so counts are derived and only validation issues are stamped. And **a two-file version rule is a rule that gets half-followed**, which is why `version.ts` had drifted five minor versions behind. Web app details in `web/CHANGELOG.md` (0.11.0–0.13.0).

- Corrected the release procedure in `AGENTS.md`. It had instructed every web app release to bump the version in both `web/package.json` and `web/src/version.ts`, and a two-file rule is a rule that gets half-followed: `version.ts` sat unread at `0.5.1` against a `0.10.0` package, with **no importers anywhere in the repository**. `CR-030` deleted it and the stamp now reads the package version at build time, so the instruction names one file because there is one. Recorded here rather than only in `web/CHANGELOG.md` because the fault was in the guidance, not in the app.

- Completed `CR-014`, three months after it was filed, by discovering that its question no longer had a subject. The request asked where `web/scripts/image-manifest.json` should live and whether it should be committed, on the premise that it was a working regeneration cache. It was not: `generate-images.ts` drops every item whose frontmatter carries `image:` at line 211, and loads the manifest at line 223, so the cache is consulted after the filter that removes everything it describes. Measured on the real tree — all 21 keys resolve to content files, and all 21 of those files now carry `image:`, fifteen from `CR-034`'s backfill and six from the first real generation run. No lookup could hit, and none can arise, because a successful generation writes back the frontmatter that disqualifies the item next time. A hit now requires the write-back to have failed or an author to have hand-deleted `image:` — and in that second case suppressing regeneration is wrong anyway, since deleting those lines is how you ask for a new image.

  Replaced by `web/scripts/image-log.json`: an append-only array of `{ section, slug, generatedAt, prompt, images }`, holding the same 21 items migrated in chronological order with `contentHash` dropped. What survives is exactly what frontmatter does not duplicate — how each image was actually made. An array rather than a map because the manifest overwrote an item's row on regeneration, discarding the earlier prompt and URL; successive generations are successive facts, not corrections. `parseLog` throws on malformed input instead of warning and starting fresh, which is right for a cache and data loss for a record.

  It stays committed, against a request that concluded generated artifacts should not be, on a rule restated as **reproducibility** rather than generatedness. `posts-data.ts` and the synced `content/stories/` are pure functions of Markdown, rebuilt on every deploy, and rollback is by known-good SHA (`CR-019`); a non-deterministic, paid model call producing an object in a bucket is reconstructible at no price. Orphan detection was considered and deliberately not added: an entry naming deleted content is a true statement about the past, and a record that answers no questions cannot serve a stale answer. `docs/wiki/projects/content-pipeline.md` records the boundary. Web app details in `web/CHANGELOG.md` (0.10.0).

- Completed `CR-035`, the same day it was filed. `generate:images` now skips `draft: true` posts by default. The first open question — whether skipping is the default or whether naming a draft on the command line is itself the opt-in — was settled by reading the filter rather than by preference: item selection is a substring match, so `posts/my` selects both `my-journal-journey` and `my-wandering-mind`, and consent that is sometimes accidental is not a safeguard. Hence an explicit `--include-drafts`. The premise was confirmed with the repository owner before implementing, because it was reversible: had a draft been meant to be reachable on the web, generating its image would have been correct and the request void. The skip is announced, naming each draft and the flag that releases it. Verified against a fixture, because the real content tree can no longer reach the defect — every post now carries `image:` frontmatter, so the `hasCustomImage` filter empties the list first. Web app details in `web/CHANGELOG.md` (0.9.0).

- Completed `CR-034`. Hero images are live again — 15 of 15, verified by fetching each page from production and matching its expected R2 URL. The content backfill alone achieved that, with no application change at all: `deploy.yml` ran the same plain `build:posts` it always has, because the URLs now arrive as ordinary frontmatter. What followed is what stops it recurring. `generate:images` had hardcoded the pre-split in-repo content path, which now holds a single `README.md`, so it discovered zero items and had been silently inert since `CR-020`; it resolves `CONTENT_DIR` now and writes its URLs back into the source Markdown, which is the step that makes a generated image durable. `ImageGeneratorProcessor` is deleted: image generation was implemented twice, and the processor was the copy that made images a build-time concern — with URLs in content, a build needs no image machinery at all.

- The work uncovered a larger gap than the defect. `web/scripts/` — which holds `build-posts.ts`, the entire content build — was covered by **no tsconfig and no test runner**. `web/tsconfig.json` excludes `scripts`, the root program excludes `web`, and both test globs stop short of it; confirmed with `tsc --listFiles` against all three programs. So `build-posts.ts` had never been typechecked, including through `CR-013` and `CR-028`, which both modified it. A new `web/tsconfig.scripts.json` found a standing error on its first run — `standalonePageSources` is `as const`, so `new Set(...)` inferred `Set<'pages'>` and membership tests against arbitrary directory names did not compile. `typecheck:web-scripts` is now in the root chain and in `app-ci.yml`, and `test:scripts` covers `web/scripts/utils/*.test.ts`. Web app details in `web/CHANGELOG.md` (0.8.0).
