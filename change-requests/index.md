...
- Reshaped `CR-012` from "Decide parser roadmap for Markdown processing" to "Retire duplicate Markdown parsing", and gave it the detail file it never had. Gate 2 found the roadmap question answered by implementation — `marked` for AST and HTML, `gray-matter` for frontmatter, seven processors in a documented order, with `docs/wiki/concepts/markdown-processing.md` already recording the direction — and its one concrete open question orphaned, since the browser-safe `gray-matter` shim that `CR-011` deferred here existed for the admin preview that `CR-029` deleted. What reconnaissance found underneath is smaller and real: `web/src/utils/markdown.ts` still exports a hand-rolled `extractFrontmatter` and `parseMarkdownContent` with no callers, and it is not equivalent to the live parser — it coerces nothing, so `draft: true` becomes the string `"true"` while `DraftFilterProcessor` tests for the boolean, meaning anything adopting it would silently publish drafts. Alongside it, six of eleven `post-cache.ts` exports are unused, the `PostMetadata` alias is unused, and `@types/marked` is pinned at a version published deprecated as a stub because `marked` ships its own types. The ID is stable; only the scope changed.
