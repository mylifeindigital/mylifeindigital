...
- Settled `CR-024`'s third open question: `technical-sessions` keeps the default treatment and `stories` is the only section that diverges, so the request ships one theme and its claim that a second costs only token values stays testable. Expanded the remaining logo question with what phase 2 cannot reach: the mark's colours are literal hex in the inline SVG rather than CSS, and its tile stops are exactly `--bg-dark`/`--bg-darker`, so the tile is invisible camouflage today and becomes a cool-navy square against a warm story background.
