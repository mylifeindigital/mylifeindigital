...
- Added `CR-031` after checking, prompted by a question about whether `post-cache.ts` was doing Cloudflare's caching, what the site's caching actually is. It is not doing that job — `post-cache.ts` builds four in-memory `Map` indexes over the generated `posts-data.ts` at module load, with no Cache API, no KV, and no headers anywhere in `web/src`. Production confirms the picture: HTML responses carry **no `cache-control` at all**, so every page view executes the Worker and downstream caching is heuristic rather than chosen; and `/styles/main.css` is edge-cached (`cf-cache-status: HIT`) but served `max-age=0, must-revalidate`, so every page load revalidates. That default is currently load-bearing — it is why the `CR-024` story theme appeared the moment the deploy finished — and it is also why a longer TTL cannot simply be set: the stylesheet is served from a fixed path, so a cached copy would go stale on the next deploy. The request should decide an explicit HTML header, whether to fingerprint `main.css` so it can be cached for a year and still ship instantly, and whether HTML is worth edge-caching at all given that rendering is a Map lookup against memory. The naming is a separate small cost: `post-cache.ts` reads as a CDN concern and is a lookup index, which is what made the question worth asking.
